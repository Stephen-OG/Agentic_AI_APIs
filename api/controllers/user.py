...
async def create_character(character: Character):
    new_character = await collection["characters"].insert_one(character.model_dump())
    created_character = await collection["characters"].find_one({"_id": new_character.inserted_id})
    return character_serializer(created_character)

# Read Character by ID
@router.get("/characters/")
async def get_characters():
    characters = await collection["characters"].find().to_list(100)
    if not characters:
